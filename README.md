...
The reviewer demo uses a saved fictional US institutional-philanthropy example generated by an actual ChatGPT run, so the full worklist renders immediately without a provider call. The visible criteria and applicant fields remain editable, and **Run reviewer triage** starts a separate live provider analysis.
...
The reviewer demo has its own first-click URL:

```text
https://grant-fit-auditor.onrender.com/?reviewer-demo=1
```

It opens reviewer mode and immediately renders the saved fictional Riverbend Opportunity Fund worklist.
